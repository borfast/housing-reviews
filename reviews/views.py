...
class AgencyReviewDetailView(DetailView):
    model = AgencyReview
    ...
    def get_context_data(self, **kwargs):
        # Call the base implementation first to get a context
        context = super(AgencyReviewDetailView, self)\
            .get_context_data(**kwargs)
        # Add in the votes for this review
        (review_score, ignore) = AgencyReviewScore.objects.get_or_create(
            review=self.object, defaults={'score': 0, 'total_votes': 0})
        if review_score.score < 0:
            review_score.score = 0

        try:
            vote = AgencyReviewVote.objects.get(review=self.object,
...
class AgencyReviewCreateView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        agency.rating.add(score=form.instance.rating, user=self.request.user,
...
class AgencyReviewVoteView(LoginRequiredMixin, AjaxResponseMixin,
    ...
    def post_ajax(self, request, *args, **kwargs):
        ...
        (review_vote, created) = AgencyReviewVote.objects\
            .get_or_create(review_id=self.kwargs['review_id'],
        ...
        review_score = AgencyReviewScore.objects\
            .get(review_id=self.kwargs['review_id'])
