...
class InvitationKeyManager(models.Manager):
    def get_key(self, invitation_key):
        ...
        """
        # Don't bother hitting database if invitation_key doesn't match an SHA1 pattern.
    ...
    def create_invitation(self, user, email):
        ...
        key = hashlib.sha1("%s%s%s" % (datetime.datetime.now(), salt, user.username)).hexdigest()
        return self.create(from_user=user, key=key, email=email)
...
class InvitationKey(models.Model):
    ...
    def key_expired(self):
        ...
        return self.date_invited + expiration_date <= datetime_now()
    ...
    def send_to(self, email):
        ...
        subject = render_to_string('come_in/invitation_email_subject.txt',
                                   { 'site': current_site,
                                     'invitation_key': self })
        # Email subject *must not* contain newlines
        subject = ''.join(subject.splitlines())

        message = render_to_string('come_in/invitation_email.txt',
                                   { 'invitation_key': self,
...
# Only create invite users if configured for that
if getattr(settings, 'CREATE_INVITE_USERS', False):
